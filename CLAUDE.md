...
- `src/tools/` — pointer/wheel input → draw / erase / line / pan / zoom / block / path
  (`tools.ts`). Brush strokes interpolate along the path (no gaps on fast moves) and clone the
  instances map once per stroke.
- `src/commands/` — Command pattern → undo/redo; a stroke coalesces into one step.
- `src/features/` — `library`, `palette`, `placement`, `noise`, `divider` (block-aware),
  `svgImport`, `audio`, `halftone` (image / GIF / video → shapes, animated), and the stencil
  sources (`stencil` → `litFn` per cell, `stencilImage`, `stencilText`).
...
- **Safari / WebKit (WIP)**: no `ImageDecoder` (animated GIF falls back to a play-only `<img>`,
  ...
  content, so an empty-canvas zoom needs the div hit-target. Wheel deltas are normalized + floored
  (Safari/Firefox report line-mode or tiny deltas).
